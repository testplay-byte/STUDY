...
- **Figure crops** live in `Books/Digital/assets/` (`<BATCH>-<NNN>-fig-<slug>.png` plus
  special crops like `M1-023-keyfacts-icon.png`, `M1-001-unit-banner.png`), cut from the
  raw scans with `tools/crop-figure.py` (fractional boxes). Locally referenced images must
  stay inside `assets/` (enforced).
- **Each page is self-contained** (inline CSS; KaTeX via CDN) with a slim toolbar and a
  colophon linking the original scan (`../Raw/<Subject>/<Chapter>/<NNNN>.jpg`) and the
  markdown source (`../Formatted/<Subject>/Chapter-NN-…/page-NNN.md`).
- **Hand-typeset, never regenerated by a script.** There is no generator: pages are
  ...
  remains the canonical library; Digital is a test edition. Scope + links + asset
  containment are enforced by `bun tools/check-digital-test.mjs` (ALL GREEN required).
